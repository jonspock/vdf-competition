...
inline void reduce(form& f) {
  ...
  gmp_printf("new a after 1st normalize %Zx\n", f.a);
  gmp_printf("new b after 1st normalize %Zx\n", f.b);
  gmp_printf("new c after 1st normalize %Zx\n", f.c);

  while ((mpz_cmp(f.a, f.c) > 0) || (mpz_cmp(f.a, f.c) == 0 && mpz_cmp_si(f.b, 0) < 0)) {
    mpz_add(s, f.c, f.b);

    gmp_printf("in reduce s for div %Zx\n", s);

    // x = 2c
    mpz_mul_ui(x, f.c, 2);

    gmp_printf("in reduce 2*c for div %Zx\n", x);

    mpz_fdiv_q(s, s, x);

    gmp_printf("in reduce s %Zx\n", s);
  ...
  gmp_printf("new a before final normalize %Zx\n", f.a);
  gmp_printf("new b before final normalize %Zx\n", f.b);
  gmp_printf("new c before final normalize %Zx\n", f.c);
...
inline form generator_for_discriminant(mpz_t* d) {
  ...
  gmp_printf("for gen c (hex = %Zx\n", x.c);

  reduce(x);

  gmp_printf("for reduced a (hex = %Zx)\n", x.a);
  gmp_printf("for reduced b (hex = %Zx)\n", x.b);
  gmp_printf("for reduced c (hex = %Zx)\n", x.c);
...
inline int solve_linear_congruence(mpz_t& mu, mpz_t& a, mpz_t& b, mpz_t& m) {
  // gmp_printf("for gcdext a = %Zx\n", a);
  // gmp_printf("for gcdext m = %Zx\n", m);
...
inline form* square(form& f1) {
  ...
  gmp_printf("new a before reduce = %Zx\n", f3.a);
  ...
  gmp_printf("new b before reduce = %Zx\n", f3.b);
  gmp_printf("new c before reduce = %Zx\n", f3.c);
...
inline form repeated_square(form* f, uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) { f = square(*f); }
  return *f;
}

int main(int argc, char* argv[]) {
  timer time_calc;
  mpz_inits(negative_a, r, denom, old_a, old_b, ra, s, x, g, d, e, q, w, m, u, a, b, k, mu, v, sigma, lambda, f3.a,
            f3.b, f3.c, f3.d, NULL);

  mpz_t discriminant;
  int ret = mpz_init_set_str(discriminant,
                             "-1174860813617189793376687866595681257566773109162648214657033493857291770300456932033842"
                             "4482476778417328473277751253005897383252417809294541920707334384671",
                             0);
  ...
  time_calc.stop();
  std::cout << "Time Elapsed for repeated_square(): " << time_calc.duration_in_milliseconds(iterations)
