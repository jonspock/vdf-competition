...
#ifdef DEB        
        std::cout << "b = " << f.b << " b_exp = " << b_exp << " b= " << b << "\n";
#endif
//        std::cout << "Exp = " << a_exp << "," << b_exp << "," << c_exp << "," << max_exp - min_exp << "\n";
...
int main(int argc, char* argv[]) {
    ...
    std::string D("-283178161896406766256338548801118480561159510501921419471614013957318883445267258354383871905867382312137"
    ...
    int ret = mpz_init_set_str(discriminant, D.c_str(), 0);
    assert(ret == 0);
    form x = generator_for_discriminant(&discriminant);

    char * tmp = mpz_get_str(NULL,10,x.c);
    std::string c_str = tmp;

    relic_int one(1);
    relic_int two(2);
    relic_int disc(c_str);
        
    Classgroup g(two, one, disc);
    ClassgroupSquarer Sqr(g);
    
    uint64_t iterations = 100000;// 11;
    
    for (int j = 0; j < 1; j++) {
      time_calc.start();
      repeated_square(&x, iterations);
      time_calc.stop();
        std::cout << "Time Elapsed for Dutta.square(): " << time_calc.duration_in_milliseconds(iterations)
        << " ms per iteration\t" << time_calc.duration_in_milliseconds() << " ms total \n";

        time_calc.start();
        for (int i = 0; i < iterations; i++) Sqr.square();
        time_calc.stop();
        std::cout << "Time Elapsed for Sqr.square(): " << time_calc.duration_in_milliseconds(iterations)
        << " ms per iteration\t" << time_calc.duration_in_milliseconds() << " ms total \n";
    }
