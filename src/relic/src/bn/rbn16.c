...
void bn_mul_comba(bn_t t, const bn_t a, const bn_t b) {
  ...
  if (init == 0) {
    for (int i=0;i<16;i++) 
      for (int j=0;j<2048;j++) hist[i][j] = 0;
    init = 1;
  }
  int a_bits = bn_bits(a);
  int b_bits = bn_bits(b);
  
  int max = (a_bits>b_bits) ? a_bits : b_bits;
  int min = (a_bits<b_bits) ? a_bits : b_bits;
  if (min > 15) min = 15;
  if (max > 2047) max = 2047;
  hist[min][max] = hist[min][max] + 1;
  call_count++;
  if (call_count == (1<<22)) {
    for (int i=0;i<16;i++) {
      for (int j=0;j<2048;j++) printf("%d ",hist[i][j]);
      printf("\n");
    }
  }
  //printf("%d %d\n",a->used, b->used);
