...
static void bn_div_imp(bn_t c,
  ...
  if (init == 0) {
    for (int i=0;i<32;i++) 
      for (int j=0;j<32;j++) hist[i][j] = 0;
    init = 1;
  }
  int max = (a->used>b->used) ? a->used : b->used;
  int min = (a->used<b->used) ? a->used : b->used;
  hist[min][max] = hist[min][max] + 1;
  call_count++;
  if (call_count == (1<<20)) {
    for (int i=0;i<32;i++) {
      for (int j=0;j<32;j++) printf("%d ",hist[i][j]);
      printf("\n");
    }
  }
	/* If |a| < |b|, we're done. */
 ...
 * @param[in] b			- the the divisor.
 */
static void bn_div_imp_with_temps(bn_t c, bn_t d,
 ...
 * @param[in] b			- the the divisor.
 */
static void bn_div_imp1_const(bn_t c,
                              bn_t xx, bn_t yy, bn_t rr, bn_t qq,
                              const bn_t a, const bn_t b) {
	int sign;
