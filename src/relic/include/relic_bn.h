...
/**
 * Assigns a small negative constant to a multiple precision integer.
 *
 * The constant must fit on a multiple precision digit, or dig_t type using
 * only the number of bits specified on BN_DIGIT.
 * The input itself will be positive, but the sign of the output will indicate
 * a negative value (check for zero may be needed)
 *
 * @param[out] a            - the result.
 * @param[in] digit         - the positive value of constant to assign.
*/
...
void bn_div_with_temps(bn_t c,
            ...
            const bn_t a, const bn_t b);
void bn_div_fast(bn_t c, bn_t rr,  bn_t a, bn_t b);
...
void bn_gcdext(bn_t b, bn_t a, bn_t c, bn_t d);
